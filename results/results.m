...
for t=1:length(L)
    ...
    for del=1.2:-0.1:0.1
        ...
        for it=1:100
            ...
            if min(error_p(error_r>0))<=1e-6
                prob_r(c,t) = prob_r(c,t) + 1;
            end
...
subplot(2,2,2*(t-1)+2),imagesc(prob_p/100),colormap gray,title('Pytch'),...
    xlabel('L','FontSize',16); ylabel('delta','FontSize',16);
subplot(2,2,2*(t-1)+3),imagesc(prob_r/100),colormap gray,title('RANA'),...
    xlabel('L','FontSize',16); ylabel('delta','FontSize',16);
...
subplot(1,2,1),plot(time*1e15,abs(x),time*1e15,abs(z_s),time*1e15,abs(z_p),time*1e15,abs(z_r)),title('Reconstructed magnitud'),...
    xlabel('Time [fsec]','FontSize',16); ylabel('Amplitude','FontSize',16);
subplot(1,2,2),plot(time*1e15,unwrap(angle(x)),time*1e15,unwrap(angle(z_s)), time*1e15,unwrap(angle(z_p)),time*1e15,unwrap(angle(z_r))),...
