...
if isfield(Params, 'T')           == 0,  Params.T           = 120;              end
...
tic
[z,error,mec] = solver(Ynoisy,x,Params, A,1e-10);
toc

z      = best_sol(z, x);
delta  = acos(abs(z'*x)/sqrt( (z'*z)*(x'*x) ));
mecm   = min(mec);
...
subplot(4,2,8);semilogy(0:T,mec),xlabel('Iteration'), ylabel('Metric Relative error (log10)'), ...
title('Relative error vs. iteration count')